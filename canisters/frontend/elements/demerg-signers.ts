...
import { _SERVICE, SignerProposal } from '../dfx_generated/backend/backend.did';
...
export class DemergSigners extends HTMLElement {
        ...
        try {
            const description = (
                this.shadow.querySelector(
                    '#input-signer-proposal-description'
                ) as any
            ).value as string;
            const signer = (
                this.shadow.querySelector(
                    '#input-signer-proposal-signer'
                ) as any
            ).value as string;
            const remove = (
                this.shadow.querySelector(
                    '#input-signer-proposal-remove'
                ) as any
            ).checked as boolean;

            const result = await this.store.backend.propose_signer(
                description,
                Principal.fromText(signer),
                remove
            );
    ...
    async handle_vote_on_signer_proposal_click(
        ...
        try {
            const result = await this.store.backend.vote_on_signer_proposal(
                signerProposalId,
                adopt
            );
    ...
    async load_signers() {
        ...
        this.dispatchEvent(
            new CustomEvent('signers-changed', {
                detail: {
                    loading: false,
                    value: signers
                }
    ...
    async load_signer_proposals() {
        ...
        const signer_proposals =
            await this.store.backend.get_signer_proposals();

        this.store.signer_proposals =
            sort_created_at_descending(signer_proposals);
    }
        ...
        return html`
            ...
            <ui5-card>
                <ui5-card-header title-text="Signers">
                    ...
                    >
                        <ui5-button
                            ...
                            design="Emphasized"
                            @click=${() =>
                                (this.store.hide_create_signer_proposal =
                                    false)}
                        >
                            Create Proposal
                        </ui5-button>
                        ...
                        <ui5-busy-indicator
                            size="Small"
                            .active=${state.loading_signers ||
                            state.loading_signer_proposals}
                            delay="0"
                        >
                            <ui5-button
                                @click=${async () => {
                ...
                <ui5-table
                    ...
                    no-data-text="No ${state.hide_open_signer_proposals === true
                        ? 'closed'
                        : 'open'} proposals"
                >
                    ${state.hide_open_signer_proposals === false
                        ? html`
                              <ui5-table-column
                                  slot="columns"
                              ></ui5-table-column>
                    ...
                    ${state.signer_proposals
                        .filter((signer_proposal) => {
                            const open =
                                signer_proposal.adopted === false &&
                                signer_proposal.rejected === false;
                            const hidden =
                                (open &&
                                    state.hide_open_signer_proposals ===
                                        true) ||
                                (!open &&
                                    state.hide_open_signer_proposals === false);
                        ...
                        .map((signer_proposal) => {
                            const id_trimmed = `${signer_proposal.id.slice(
                                0,
                                5
                            )}...${signer_proposal.id.slice(
                                signer_proposal.id.length - 5,
                                signer_proposal.id.length
                            )}`;
                            const proposer_trimmed = `${signer_proposal.proposer
                                .toString()
                                .slice(0, 5)}...${signer_proposal.proposer
                                .toString()
                                .slice(
                                    signer_proposal.proposer.toString().length -
                                        5,
                                    signer_proposal.proposer.toString().length
                                )}`;

                            const votes_for = signer_proposal.votes.filter(
                                (vote) => vote.adopt === true
                            ).length;
                            const votes_against = signer_proposal.votes.filter(
                            ...
                            return html`
                                <ui5-table-row>
                                              ...
                                              <ui5-table-cell>
                                                  ...
                                                  >
                                                      <ui5-button
                                                          design="Positive"
                                                          @click=${() =>
                                                              this.handle_vote_on_signer_proposal_click(
                                                                  signer_proposal.id,
                                                                  true
                                                              )}
                                                      >
                                    ...
                                    <ui5-table-cell>
                                        <ui5-label title="${signer_proposal.id}"
                                    ...
                                    <ui5-table-cell>
                                        <ui5-label
                                            title="${signer_proposal.proposer}"
                                    ...
                                    <ui5-table-cell>
                                        <ui5-label
                                            >${signer_proposal.description}</ui5-label
                                        >
                                    </ui5-table-cell>

                                    <ui5-table-cell>
                                        <ui5-label
                                            >${signer_proposal.signer}</ui5-label
                                        >
                                    </ui5-table-cell>
            ...
            ${state.hide_create_signer_proposal === false
                ? html`
                      <ui5-dialog
                          ...
                          <section class="demerg-input-form">
                              <div class="demerg-input">
                                  <div class="demerg-input-and-label">
                                      <ui5-label
                                          for="input-signer-proposal-description"
                                          required
                                          >Description:</ui5-label
                                      >
                                      <ui5-input
                                          id="input-signer-proposal-description"
                                      ></ui5-input>
                                  </div>
                              </div>
                              ...
                              <div class="demerg-input">
                                  <div class="demerg-input-and-label">
                                      <ui5-label
                                          for="input-signer-proposal-signer"
                                          required
                                          >Signer:</ui5-label
                                      >
                                      <ui5-input
                                          id="input-signer-proposal-signer"
                                      ></ui5-input>
                                  </div>
                              </div>
                              ...
                              <div class="demerg-input">
                                  <sp-theme scale="medium">
                                      <sp-radio-group
                                          ...
                                          vertical
                                      >
                                          <sp-radio value="add-signer"
                                              >Add signer</sp-radio
                                          >
                                          <sp-radio
                                              id="input-signer-proposal-remove"
                                              value="remove-signer"
                                              >Remove signer</sp-radio
                                          >
                                      </sp-radio-group>
                                  </sp-theme>
                              </div>
                          ...
                          >
                              ...
                              >
                                  <ui5-button
                                      design="Emphasized"
                                      @click=${() =>
                                          this.handle_create_signer_proposal_click()}
                                  >
                                      Create
                                  </ui5-button>
                              </ui5-busy-indicator>
