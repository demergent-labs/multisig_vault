...
class DemergThreshold extends HTMLElement {
        ...
        try {
            const description = (
                this.shadow.querySelector(
                    '#input-threshold-proposal-description'
                ) as any
            ).value as string;
            const threshold = (
                this.shadow.querySelector(
                    '#input-threshold-proposal-threshold'
                ) as any
            ).value as number;

            const result = await this.store.backend.propose_threshold(
                description,
                threshold
            );
    ...
    async handle_vote_on_threshold_proposal_click(
        ...
        try {
            const result = await this.store.backend.vote_on_threshold_proposal(
                thresholdProposalId,
                adopt
            );
    ...
    async load_threshold_proposals() {
        ...
        const threshold_proposals =
            await this.store.backend.get_threshold_proposals();

        this.store.threshold_proposals =
            sort_created_at_descending(threshold_proposals);
    }
    ...
    render(state: State) {
        const threshold_subtitle_text =
            state.threshold.loading === true || state.signers.loading === true
                ? 'Loading...'
                : `${state.threshold.value} of ${state.signers.value.length} signers required`;
        ...
        return html`
            ...
            <ui5-card>
                <ui5-card-header
                    ...
                    subtitle-text="${threshold_subtitle_text}"
                >
                    <div
                    ...
                    >
                        <ui5-button
                            ...
                            design="Emphasized"
                            @click=${() =>
                                (this.store.hide_create_threshold_proposal =
                                    false)}
                        >
                            Create Proposal
                        </ui5-button>
                        ...
                        <ui5-button
                            ...
                            @click=${async () => {
                                this.store.hide_open_threshold_proposals =
                                    false;
                        ...
                        >
                            <ui5-button
                                @click=${async () => {
                                    this.store.loading_threshold_proposals =
                                        true;
                ...
                <ui5-table
                    ...
                    no-data-text="No ${state.hide_open_threshold_proposals ===
                    true
                        ? 'closed'
                        : 'open'} proposals"
                >
                    ${state.hide_open_threshold_proposals === false
                        ? html`
                              <ui5-table-column
                                  slot="columns"
                                  demand-popin
                    ...
                    ${state.threshold_proposals
                        .filter((threshold_proposal) => {
                            const open =
                                threshold_proposal.adopted === false &&
                                threshold_proposal.rejected === false;
                            const hidden =
                                (open &&
                                    state.hide_open_threshold_proposals ===
                                        true) ||
                                (!open &&
                                    state.hide_open_threshold_proposals ===
                                        false);
                        ...
                        .map((threshold_proposal) => {
                            const id_trimmed = `${threshold_proposal.id.slice(
                                0,
                                5
                            )}...${threshold_proposal.id.slice(
                                threshold_proposal.id.length - 5,
                                threshold_proposal.id.length
                            )}`;
                            const proposer_trimmed = `${threshold_proposal.proposer
                                .toString()
                                .slice(0, 5)}...${threshold_proposal.proposer
                                .toString()
                                .slice(
                                    threshold_proposal.proposer.toString()
                                        .length - 5,
                                    threshold_proposal.proposer.toString()
                                        .length
                                )}`;

                            const votes_for = threshold_proposal.votes.filter(
                                (vote) => vote.adopt === true
                            ).length;
                            const votes_against =
                                threshold_proposal.votes.filter(
                            ...
                            return html`
                                <ui5-table-row>
                                              ...
                                              <ui5-table-cell>
                                                  ...
                                                  >
                                                      <ui5-button
                                                          design="Positive"
                                                          @click=${() =>
                                                              this.handle_vote_on_threshold_proposal_click(
                                                                  threshold_proposal.id,
                                                                  true
                                                              )}
                                                      >
                                    ...
                                    <ui5-table-cell>
                                        <ui5-label
                                            title="${threshold_proposal.proposer}"
                                    ...
                                    <ui5-table-cell>
                                        <ui5-label
                                            >${threshold_proposal.description}</ui5-label
                                        >
                                    </ui5-table-cell>

                                    <ui5-table-cell>
                                        <ui5-label
                                            >${threshold_proposal.threshold}</ui5-label
                                        >
                                    </ui5-table-cell>
            ...
            ${state.hide_create_threshold_proposal === false
                ? html`
                      <ui5-dialog
                          ...
                          <section class="demerg-input-form">
                              <div class="demerg-input">
                                  <div class="demerg-input-and-label">
                                      <ui5-label
                                          for="input-threshold-proposal-description"
                                          required
                                          >Description:</ui5-label
                                      >
                                      <ui5-input
                                          id="input-threshold-proposal-description"
                                          style="margin-bottom: .75rem"
                                      ></ui5-input>
                                      <ui5-label
                                          for="input-threshold-proposal-threshold"
                                          required
                                          >Threshold:</ui5-label
                                      >
                                  </div>
                          ...
                          >
                              ...
                              >
                                  <ui5-button
                                      design="Emphasized"
                                      @click=${() =>
                                          this.handle_create_threshold_proposal_click()}
                                  >
                                      Create
                                  </ui5-button>
                              </ui5-busy-indicator>
