...
class DemergTransfers extends HTMLElement {
        ...
        try {
            const description = (
                this.shadow.querySelector(
                    '#input-transfer-proposal-description'
                ) as any
            ).value as string;
            const destination_address = (
                this.shadow.querySelector(
                    '#input-transfer-proposal-destination-address'
                ) as any
            ).value as string;
            const amount_string = (
                this.shadow.querySelector(
                    '#input-transfer-proposal-amount'
                ) as any
            ).value.toString() as string;
            const decimals = amount_string?.split('.')[1]?.length ?? 0;
            const amount =
                BigInt(Number(amount_string ?? 0) * 10 ** decimals) *
                BigInt(10 ** (8 - decimals));

            const result = await this.store.backend.propose_transfer(
                description,
                destination_address,
                amount
            );
            ...
            if (result.hasOwnProperty('ok')) {
                this.load_transfer_proposals();
                (
                    this.shadow.querySelector('#toast-proposal-created') as any
                ).show();
            } else {
    ...
    async handle_vote_on_transfer_proposal_click(
        ...
        try {
            const result = await this.store.backend.vote_on_transfer_proposal(
                transferProposalId,
                adopt
            );
            ...
            if (result.hasOwnProperty('ok')) {
                ...
                this.load_transfer_proposals();
                (
                    this.shadow.querySelector('#toast-vote-recorded') as any
                ).show();
            } else {
                this.handle_error((result as any).err);
            }
    ...
    async load_balance() {
        ...
        const vault_balance_result =
            await this.store.backend.get_vault_balance();
    ...
    async load_transfer_proposals() {
        ...
        const transfer_proposals =
            await this.store.backend.get_transfer_proposals();

        this.store.transfer_proposals =
            sort_created_at_descending(transfer_proposals);
    }
    ...
    render(state: State) {
        const canister_balance_text =
            state.balance.loading === true
                ? 'Loading...'
                : `${
                      Number((state.balance.value * 10000n) / BigInt(10 ** 8)) /
                      10000
                  } ICP available`;
        ...
        return html`
            ...
            <ui5-card>
                <ui5-card-header
                    ...
                    subtitle-text="${canister_balance_text}"
                >
                    <div
                    ...
                    >
                        <ui5-button
                            ...
                            design="Emphasized"
                            @click=${() =>
                                (this.store.hide_create_transfer_proposal =
                                    false)}
                        >
                            Create Proposal
                        </ui5-button>
                        ...
                        <ui5-busy-indicator
                            size="Small"
                            .active=${state.balance.loading ||
                            state.loading_transfer_proposals}
                            delay="0"
                        >
                            <ui5-button
                                @click=${async () => {
                                    ...
                                    this.store.loading_transfer_proposals =
                                        true;
                ...
                <ui5-table
                    ...
                    no-data-text="No ${state.hide_open_transfer_proposals ===
                    true
                        ? 'closed'
                        : 'open'} proposals"
                >
                    ${state.hide_open_transfer_proposals === false
                        ? html`
                              <ui5-table-column
                                  slot="columns"
                              ></ui5-table-column>
                    ...
                    ${state.transfer_proposals
                        ...
                        .map((transfer_proposal) => {
                            const id_trimmed = `${transfer_proposal.id.slice(
                                0,
                                5
                            )}...${transfer_proposal.id.slice(
                                transfer_proposal.id.length - 5,
                                transfer_proposal.id.length
                            )}`;
                            const proposer_trimmed = `${transfer_proposal.proposer
                                .toString()
                                .slice(0, 5)}...${transfer_proposal.proposer
                                .toString()
                                .slice(
                                    transfer_proposal.proposer.toString()
                                        .length - 5,
                                    transfer_proposal.proposer.toString().length
                                )}`;

                            const votes_for = transfer_proposal.votes.filter(
                                (vote) => vote.adopt === true
                            ).length;
                            const votes_against =
                                transfer_proposal.votes.filter(
                                    (vote) => vote.adopt === false
                                ).length;
                            ...
                            return html`
                                <ui5-table-row>
                                              ...
                                              <ui5-table-cell>
                                                  ...
                                                  >
                                                      <ui5-button
                                                          design="Positive"
                                                          @click=${() =>
                                                              this.handle_vote_on_transfer_proposal_click(
                                                                  transfer_proposal.id,
                                                                  true
                                                              )}
                                                      >
                                    ...
                                    <ui5-table-cell>
                                        <ui5-label
                                            title="${transfer_proposal.id}"
                                            >${id_trimmed}</ui5-label
                                        >
                                    </ui5-table-cell>
                                    ...
                                    <ui5-table-cell>
                                        <ui5-label
                                            >${new Date(
                                                Number(
                                                    transfer_proposal.created_at /
                                                        1000000n
                                                )
                                            ).toLocaleString()}</ui5-label
                                        >
                                    </ui5-table-cell>

                                    <ui5-table-cell>
                                        <ui5-label
                                            title="${transfer_proposal.proposer}"
                                            >${proposer_trimmed}</ui5-label
                                        >
                                    </ui5-table-cell>

                                    <ui5-table-cell>
                                        <ui5-label
                                            >${transfer_proposal.description}</ui5-label
                                        >
                                    </ui5-table-cell>

                                    <ui5-table-cell>
                                        <ui5-label
                                            >${transfer_proposal.destination_address}</ui5-label
                                        >
                                    </ui5-table-cell>
            ...
            ${state.hide_create_transfer_proposal === false
                ? html`
                      <ui5-dialog
                          ...
                          <section class="demerg-input-form">
                              <div class="demerg-input">
                                  <div class="demerg-input-and-label">
                                      <ui5-label
                                          for="input-transfer-proposal-description"
                                          required
                                          >Description:</ui5-label
                                      >
                                      <ui5-input
                                          id="input-transfer-proposal-description"
                                      ></ui5-input>
                                  </div>
                              </div>
                              ...
                              <div class="demerg-input">
                                  <div class="demerg-input-and-label">
                                      <ui5-label
                                          for="input-transfer-proposal-destination-address"
                                          required
                                          >Destination Address:</ui5-label
                                      >
                                      <ui5-input
                                          id="input-transfer-proposal-destination-address"
                                      ></ui5-input>
                                  </div>
                              </div>
                              ...
                              <div class="demerg-input">
                                  <div class="demerg-input-and-label">
                                      <ui5-label
                                          for="input-transfer-proposal-amount"
                                          required
                                          >Amount:</ui5-label
                                      >
                          ...
                          >
                              ...
                              >
                                  <ui5-button
                                      design="Emphasized"
                                      @click=${() =>
                                          this.handle_create_transfer_proposal_click()}
                                  >
                                      Create
                                  </ui5-button>
                              </ui5-busy-indicator>
            ...
            <ui5-toast
                id="toast-proposal-created"
                placement="TopCenter"
                >Proposal Created</ui5-toast
            >
            <ui5-toast
                id="toast-vote-recorded"
                placement="TopCenter"
                >Vote Recorded</ui5-toast
            >
